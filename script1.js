...
function changeLanguageabout(language) {
    ...
    document.getElementById('abus3').textContent = translations[language].abus3;
...
// При завантаженні сторінки беремо мову з localStorage
document.addEventListener("DOMContentLoaded", function() {
    const savedLang = localStorage.getItem('language') || 'uk';
    // Якщо це about сторінка
    if (document.getElementById('aboutUsText')) {
        changeLanguageabout(savedLang);
    }
    // Якщо це contact сторінка
    if (document.getElementById('ContactText')) {
        changeLanguagecontact(savedLang);
    }
});
