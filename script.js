...
function changeLanguage(language) {
    localStorage.setItem('language', language); // Зберігаємо вибір
    var translations = {
        uk: {
            aboutUs: "Про нас",
            tours: "Тури",
...
// При завантаженні сторінки встановлюємо мову з localStorage або uk
document.addEventListener("DOMContentLoaded", function() {
    const savedLang = localStorage.getItem('language') || 'uk';
    if (document.getElementById('ToursText')) {
...
function Search() {
  ...
  for (i = 0; i < div.length; i++) {
    ...
    for (j = 0; j < divs.length; j++) {
      ...
      var image2 = divs[j].querySelector('.image2');
      
      if (filter === "") {
        divs[j].style.transition = "opacity 0.5s";
        ...
        if (image2) {
          image2.style.width = ""; // Повертаємо ширину до початкового значення
        }
      }
    }
  }
...
function darkmode() {
  ...
  searchIcon.src = !wasDarkmode ? 'search.png' : 'player.png';
}
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  searchIcon.src = isDarkmode ? 'search.png' : 'player.png';
});

    // Показати або сховати кнопку при прокручуванні сторінки
window.onscroll = function() {
  ...
  scrollButton.style.display = "none";
      ...
      // Функція прокрутки сторінки на початок
